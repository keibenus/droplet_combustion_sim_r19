    ...
    while current_t < config.T_END and status == "Running":
        ...
        while iter_count < config.MAX_INTERFACE_ITER and not converged:
            ...
            if not is_gas_phase_only:
                ...
                r_g_0_c = r_g_centers[0] if Ng > 0 else R_n + 1e-7 # Use R_n
                ...
                try:
                     ...
                     mdot_calc, q_gas_out_calc, q_liq_out_calc, v_g_surf_calc, Y_eq_calc, T_s_calc, delta_nu_final_iter = interface.solve_interface_conditions(
                         T_l_node_last=T_l_last_cell_iter, T_g_node0=T_g_first_cell_iter,
                         Y_g_node0=Y_g_first_cell_iter, P=P_n, R=R_n, # Use P_n, R_n
                         r_l_node_last_center=r_l_last_c, r_g_node0_center=r_g_0_c,
        ...
        if log_now or save_this_step or status != "Running": # Log if saved or interval or finished
             ...
             # max_dTdt_current = np.max(np.abs(dT_dt_react)) if 'dT_dt_react' in locals() and len(dT_dt_react)>0 else 0.0
        ...
        if step_count_total > 0: # 最初のステップは除く
            # 液滴の質量変化 (減少分を正とする)
            delta_m_liquid = (4.0/3.0) * np.pi * rho_l_s_for_R_update * (R_n**3 - R**3) # rho_l_s_for_R_update は Step 5 で使ったもの
